...
class MoxyTaskScheduler {
	...
	constructor(tasks?: IMoxyTask[], opts?: IMoxyTaskSchedulerOptions) {
		...
		if (tasks) {
			tasks.forEach((task: IMoxyTask, index: number) =>
	...
	public start(): boolean {
		this._monitor()
		this._masterThread = setInterval(
			() => this._monitor(),
			this._opts.idleCooldown,
		)
		return true
	}
	...
	public remove(labelOrId: string, date?: Date): boolean {
		const task = this._tasks.find(
			(task: IMoxyTask) =>
				task._id === labelOrId || task.label === labelOrId,
		)
		if (task) {
			if (date) {
				const d = task.schedule.findIndex((d: Date) => d === date)
				task.schedule = task.schedule.filter(
					(date: any, index: number) => index !== d,
				)
			} else {
				this._tasks = this._tasks.filter(
					(t: IMoxyTask) => t._id !== task._id,
				)
			}
			return true
		}
	...
	public sort(): boolean {
		this._tasks.sort((a: IMoxyTask, b: IMoxyTask) =>
	...
	public sortSchedule(task: IMoxyTask): boolean {
		task.schedule?.sort((a: Date, b: Date) =>
		...
		for (let i = 0; i < (this._opts.tasksPerIdle || 2); i++) {
			...
			if (d > job.schedule[0].valueOf()) {
				...
				if (job && job.schedule.length === 0) {
					this._tasks = this._tasks.filter(
						(task: IMoxyTask) => task._id !== job._id,
					)
				}
			} else {
				// Waiting
			...
			this.next()
			if (!this._tasks[this._queuePointer]) {
				break
			}
		}
		return true
